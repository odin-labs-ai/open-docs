...
export function mountWorkshop(host: HTMLElement, openLesson: (id: string) => void) {
  ...
  function render() {
    ...
    host.innerHTML = `<div class="workshop-welcome"><div><h2 id="workshop-entry" tabindex="-1">Make your first <br/>agent loop click.</h2><p class="workshop-introduction">One small job. Five discoveries.<br/>Learn by changing something and seeing why it works.</p></div><div class="workshop-mission"><span>Your mission</span><strong>Fix a typo. <br/>Keep the promise.</strong><p>Change <code>Welcomme</code> to <code>Welcome</code> in a help document. Preserve its FAQ link.</p><span class="rehearsal-label">Browser rehearsal · no account needed</span></div></div>
      <nav class="workshop-steps" aria-label="Workshop steps">${steps.map((s, i) => `<button data-workshop-step="${i}" ${i > saved.completed.length ? 'disabled' : ''} aria-current="${i === saved.step ? 'step' : 'false'}"><span>${saved.completed.includes(i) ? '✓' : i + 1}</span>${s.short}</button>`).join('')}</nav>
      <div class="workshop-heading"><div><p>Step ${saved.step + 1} of 5</p><h2>${step.title}</h2></div><button id="narrate-workshop" class="workshop-link">Listen to this step</button></div>
      ...
      <div class="workshop-reset"><button id="reset-workshop" class="workshop-link">Start this workshop again</button><span>Want the full system? Open Explore, Failure lab or Field guide above.</span></div>`;
  ...
  host.addEventListener('click', event => {
    ...
    switch (button.id) {
      ...
      case 'close-workshop-inspection': sceneOpen = false; scene?.pause(true); get('#workshop-inspection').hidden = true; get('#inspect-workshop-run').setAttribute('aria-expanded', 'false'); reveal(ran ? get('.workshop-evidence') : get('.workshop-task')); break;
