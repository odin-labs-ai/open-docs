...
export function mountAgentOverview(host: HTMLElement, openLesson: (id: string) => void, beginWorkshop: () => void) {
    ...
    <section class="agent-overview" aria-labelledby="agent-overview-title">
      <div class="agent-overview-lead">
        <div class="agent-overview-intro">
          ...
          <div class="agent-overview-actions"><button class="workshop-primary" id="overview-workshop">Try the hands-on workshop <span aria-hidden="true">↘</span></button><button class="workshop-link" id="overview-foundations">Read the foundations →</button></div>
        </div>
        <figure class="agent-overview-figure">
  ...
  function fallback() { if (disposed) return; get('#overview-fallback').hidden = false; get('#agent-overview-scene').hidden = true; get<HTMLButtonElement>('#overview-reset').disabled = true; }
  select(selected);
  ...
  return {
    pause(value: boolean) { paused = value; renderer?.pause(value || reduced.matches); },
    dispose() { disposed = true; reduced.removeEventListener('change', motionChanged); renderer?.dispose(); },
  };
}
