...
export function mountLessonTheater(host: HTMLElement, initial: Lesson) {
  ...
  return {
    ...
    setSection(value: 'essentials' | 'deep' | 'check', index?: number) {
      const nextSubsection = index ?? (section === value ? subsection : 0);
      if (section === value && subsection === nextSubsection) return;
      section = value; subsection = nextSubsection; stage.update(lessonStage(lesson, section, subsection));
    },
    pause: stage.pause,
